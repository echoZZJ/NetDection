...
//  sdt
//
//  Created by didi on 2020/2/12.
//

#include "t_traceRoute.h"
#include <stdio.h>
#include <stdlib.h>
//#include <limits.h>
#include <signal.h>
...
#include "comm/time_utils.h"  // comm/utils.h
...
int traceroute_info_insert(char *info)
{
    ...
    int ret = -1;
    xinfo2(TSF"net_diagno_client_sqlite_insert begin\n");
    handle = net_diagno_client_get_handle();
    ret = net_diagno_client_sqlite_insert(handle, E_TABLE_TRACEROUTE,  info);
    if (ret < 0)
    {
       xinfo2(TSF"net_diagno_client_sqlite_insert end, E_TABLE_TRACEROUTE failed\n");
        return ret;
    }
    
    xinfo2(TSF"net_diagno_client_sqlite_insert end, E_TABLE_TRACEROUTE ok\n");
...
char * traceroute_report(const char * host)
{
    ...
    {
        xerror2(TSF"getaddrinfo error: %s\n", gai_strerror(ret));
    ...
    for(parse_addr = to; parse_addr != NULL; parse_addr = parse_addr->ai_next)
    {
        ...
        {
            xerror2(TSF"create socket error: %s\n", strerror(errno));
//            goto EXIT_TRACEROUTE;
//            traceRouteBreak(to,sockfd,print_obj);
            break;
    ...
    if (sockfd < 0)
    {
      xerror2(TSF"create socket error \n");
//      goto EXIT_TRACEROUTE;
//        traceRouteBreak(to,sockfd,print_obj);
//        break;
    }
    ...
    if(parse_addr == NULL)
    {
        xerror2(TSF"traceroute, unknown host %s\n", host);
//        traceRouteBreak(to,sockfd,print_obj);
//        break;
//        goto EXIT_TRACEROUTE;
    }
...
//                   goto EXIT_TRACEROUTE;
            ...
            if(check_recv_packet(buffer, &ping_param) == 0)
            {
                if(from.sin_addr.s_addr == destination.sin_addr.s_addr)
                {
                    ...
                    xinfo2(TSF"traceroute completed\n");
                    break;
                }
...
void traceRouteBreak(struct addrinfo *to,int sockfd, print_ctrl_t * print_obj){
    if (NULL != to)
       {
           freeaddrinfo(to);
       }
    ...
    print_ctrl_t * print_obj)
{
    ...
    {
        xinfo2(TSF"Vodoo magic happened, the size of the packet must be %d a minima\n", ipheaderlen + ICMP_MINLEN);
        return;
    }
...
//    gettickcount();
    ...
    if(nbs < 0 || (unsigned int)nbs < 8 + *papram->size_data)
    {
        ...
        xinfo2(TSF"ping : sendto %s %d chars, achieve %d\n", inet_ntoa((*papram->destination).sin_addr),
        8 + *papram->size_data,
        nbs);
    }
}
...
int get_sockaddr(struct sockaddr_in * sockad, char * iface)        //get local address
{
    ...
    if(sockfd < 0)
    {
        xerror2(TSF"get_sockaddr socket error: %s\n", strerror(errno));
        return ret;
    }

    if(ioctl(sockfd, SIOCGIFCONF, &ifconf) == -1)
    {
        xerror2(TSF"ioctl error: %s\n", strerror(errno));
...
char* create_json_string_of_traceroute(print_ctrl_t *list)
{
    ...
    if(root)
    {
        ...
        if(out)
        {
            xinfo2(TSF"traceroute report: %s\n", out);
        }
