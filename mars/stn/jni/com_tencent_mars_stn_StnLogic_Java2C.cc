...
#include "mars/stn/t_stn_logic.h"

#include "stn/src/t_net_core.h"
#include "stn/src/t_net_source.h"
...
JNIEXPORT void JNICALL Java_com_tencent_mars_stn_StnLogic_startNetWorkCheck
...
DEFINE_FIND_STATIC_METHOD(KJava2C_setLonglinkSvrAddrs, KNetJava2C, "setShortlinkSvrAddrs", "([Ljava/lang/String;[ILjava/lang/String;)V")
JNIEXPORT void JNICALL Java_com_tencent_mars_stn_StnLogic_setShortlinkSvrAddrs
  (JNIEnv *_env, jclass, jobjectArray _hosts, jintArray _ports, jstring _debug_ip) {
...
//    std::string host = (NULL == _host ? "" : ScopedJstring(_env, _host).GetChar());
    ...
    if (NULL != _hosts && _env->GetArrayLength(_hosts) > 0) {
          jsize size = _env->GetArrayLength(_hosts);
          for (int i = 0; i < size; i++) {
              jstring hostItem = (jstring)_env->GetObjectArrayElement(_hosts, i);
              if (hostItem != NULL) {
                  host_list.push_back(ScopedJstring(_env, hostItem).GetChar());
              }
              _env->DeleteLocalRef(hostItem);
          }
      }
    SetShortlinkSvrAddrs(host_list, ports, debug_ip);
}
